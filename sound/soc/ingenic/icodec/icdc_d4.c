...
recheck:
	{
		int timeout = 0x5;
		int val1 = readw(icdc_d4->mapped_base + reg * sizeof(u32));
		if (value != val1) {
...
static int hpout_volsw_put(struct snd_kcontrol * kcontrol, struct snd_ctl_elem_value * ucontrol)
{
	...
	icdc_d4->hpout_vol = ucontrol->value.integer.value[0] & ICDC_CHCR_HPOUTGAIN_MSK;
...
static int alc_volsw_put(struct snd_kcontrol * kcontrol, struct snd_ctl_elem_value * ucontrol)
{
	...
	icdc_d4->mic_vol = ucontrol->value.integer.value[0] & ICDC_CACR2_ALCGAIN_MSK;
...
static int icdc_d4_adc_enable(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, ICDC_CACR2, ICDC_CACR2_ALCSEL_SIGNAL_ENDED, 0/*ICDC_CACR2_ALCSEL_SIGNAL_ENDED*/);
...
static int icdc_d4_platform_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&icdc_d4->io_lock);
